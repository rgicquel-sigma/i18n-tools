...
function writeFile(path: string, data: object): Promise<void> {
    return new Promise((resolve, reject) => {
...
function orderKeysRecursive(data: object, keepOrderDepth: number = 0): object {
...
function compareData(data: object, dataPath: string, fileName: string, refData: object, additions: Array<string>, checkOnly: boolean, exclusions: Array<Array<string>>): object {
    if (data === null || typeof(data) !== 'object') {
        return data;
    }
...
function isExcluded(key: string, exclusions: Array<Array<string>>): boolean {
...
function getNextExclusions(key: string, exclusionsArg: Array<Array<string>>): Array<Array<string>> {
...
async function processFiles(refLocale: string, pathArg: string, checkOnly: boolean, keepOrderDepth: number, exclusions: Array<Array<string>>): Promise<void> {
    ...
    for (let fileName of files) {
        if (fileName !== `${refLocale}.json`) {
            ...
            let additions = new Array<string>();
...
try {
    ...
    let exclusions = new Array<Array<string>>();
